...
class ReportView(AuthenticatedBaselView):
    ...
    def index(self, cls=None):
        ...
        room_usage = db.session.query(
            ...
            .filter(func.extract('month', Booking.check_in_date) == month) \
            .group_by(Room.style_room) \
            .all()
...
class OccupancyView(AuthenticatedBaselView):
    ...
    def index(self):
        ...
        return self.render(
            ...
            labels=[data['room_name'] for data in table_data],  # Danh sách tên phòng
            days_rented=[data['days_rented'] for data in table_data],  # Số ngày thuê
